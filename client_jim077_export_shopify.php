...
<?php
  ...
  while($row = mysqli_fetch_assoc($result)){
        ...
        $type_mod = str_replace(["rings","pendants","necklaces","bracelets","earring"],["ring","pendant","necklace","bracelet","earrings"],strtolower($row['type']));
          ...
          if ( $row['client_jim077_qty'] > 0 ) { $tags .= "MATTHEWS_STOCK"; }
          if ( $row['client_jim077_qty'] <= 0 && $row['client_sgastock'] == 1 ) { $tags .= "SGA_STOCK";}
          ...
          //price
          if ( $row['client_jim077_price'] > 0 ) { $price = $row['client_jim077_price']; }
          else { $price = $row['retail_aud'];}
          ...
          //images alt text
          if (strtolower($row['brand']) == "sapphire dreams") 
            if ( strtolower($row['type']) == "loose sapphires" && strtolower($row['treatment']) == "unheated") { $alt_text = $row['carat'] . "ct Unheated " . $row['colour'] . " " . $row['shape'] . " Australian Sapphire - Matthews Jewellers";}
            elseif ( strtolower($row['type']) == "loose sapphires" && strtolower($row['treatment']) !== "unheated") { $alt_text = $row['carat'] . "ct " . $row['colour'] . " " . $row['shape'] . " Australian Sapphire - Matthews Jewellers";}
            else { $alt_text =  ucwords($title_mod) . " " . $row['metal_composition'] . " Australian " . $row['colour'] . " Sapphire " . ucwords($type_mod) . " - Matthews Jewellers";}
          else {
            if ( strtolower($row['brand']) == "argyle origin diamonds") { $alt_text = $row['carat'] . "ct " . $row['colour'] . " " . $row['shape'] . " Argyle origin pink diamond - Matthews Jewellers";}
            elseif ( strtolower($row['brand']) == "argyle pink diamonds") { $alt_text = $row['carat'] . "ct " . $row['colour'] . " " . $row['shape'] . " Argyle certified pink diamond - Matthews Jewellers";}
            else { $alt_text = $row['product_title'] . " - Matthews Jewellers";}
          }
         ...
         foreach ($images as $index => $image) {
            $isPositionGreaterThanOne = ($index + 1 > 1);
                ...
                $content = array (
                    ...
                    29 => $isPositionGreaterThanOne ? "" : $description,
                    30 => $isPositionGreaterThanOne ? "" : $seoTitle,
                    31 => "",
                    32 => "",
                    33 => "",
                    34 => "",
  ...
  echo "<h2>JIM077: Matthews Jewellers Shopify Products Import CSV Exported!</h2><br>";
  echo "Total Products Exported to CSV: ".$count."<br><br>";
  echo "<a style='font-weight:bold;' href='https://pim.samsgroup.info/client_export/jim077_product_import.csv'>View on Web</a><br><br>";
