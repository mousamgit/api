...
class ProductApiController
{
    ...
    {
        $products = $this->getData('products/{$productId}');
        $updates=$products['product']['variants'];
        foreach($updates as $update)
        {
            return Products::where('sku',$update['sku'])->first();
        }
    }
    ...
    public function getProducts()
    {
        ...
        $products = $this->getData('products/7813970100399');

        if ($products !== false) {
            echo "Successfully fetched products data from Shopify:\n";
            dd($products['product']['variants'][0]);
        } else {
    ...
    public function createProduct()
    {
        ...
        foreach($exporting_rows as $key=> $row)
        {
            ...
            $productData = [
                "product" =>
                    [
                        'title' => 'new prd'.$row['product_title'],
