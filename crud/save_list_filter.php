...
class ProductFilterManager
{
    ...
    private function insertCase($data)
    {
        ...
        foreach ($data["attribute"] as $key => $attribute_value) {
            ...
            $sql = "INSERT INTO table_filter (`table_name`, `filter_type`, `attribute_name`, `attribute_condition`, `range_from`,`range_to`,`data_type_value`,`op_value`,`index_no`,`user_name`) 
                VALUES ('$table_name', '$filter_type', '$attribute_name', '$attribute_condition', '$range_from','$range_to','$data_type','$operator','$indexNo','$user_name')";

            if ($this->con->query($sql) == TRUE) {

                if (!($attribute_value['condition_type'] == 'group' || $attribute_value['condition_type'] == 'normal')) {
                    $update_product_filter = "update table_filter set index_no=index_no+1 where table_name='".$table_name."' AND index_no>=" . $indexNo . " and id !=" . $this->con->insert_id;
    ...
    private function updateCase($data)
    {
        ...
        foreach ($data["attribute"] as $key => $attribute_value) {
            ...
            WHERE table_name='".$table_name."' AND id = " . $attribute_value['id'];
            $this->con->query($sql);
            $success = true;
        }
        if ($success) {
